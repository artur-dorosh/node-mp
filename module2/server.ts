...
router.param('id', (req, res, next, id) => {
    req.user = users.find((user: User) => user.id === id);
    if (!req.user) {
...
router.route('/:id')
    ...
    .put(validator.body(userSchema), (req, res, next) => {
        users = users.map((item: User) => item.id === req.user.id ? req.body : item);
        res.send('Complete user update was successfully!');
        next();
    })
    .patch((req, res, next) => {
        users = users.map((item: User) => item.id === req.user.id ? {
            ...item,
            ...req.body,
        } : item);
...
function getAutoSuggestUsers(loginSubstring: string = '', limit?: number): User[] {
    return users
        .filter((item: User) => item.login.toLocaleLowerCase().includes(loginSubstring.toLocaleLowerCase()))
